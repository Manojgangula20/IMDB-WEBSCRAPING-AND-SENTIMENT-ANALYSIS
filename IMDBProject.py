...
class ImdbMovies:
    ...
    def __init__(self,**kwargs):
        ...
        self.mydb=self.DatabaseConnection('root','Honey@2017','imdbmovies')
    ...
    '''This is just to provide user friendly string when object is print'''             
    def __str__(self):
    ...
    def getMovieData(self,Movietitle):
        try:
            url = "http://www.omdbapi.com/?i=tt3896198&apikey=db80ab11".format(Movietitle)
    ...
    def DatabaseConnection(self,user, password, database):
        mydb=' '
        try:
    ...
    def DisplayCricticAudienceRating(self,movieData):
        ...
        j = j.plot_marginals(sns.histplot, kde=False,)
        j.ax_joint.text(0.1, 0.9, (lambda r, p: f"r = {r:.2f}, p = {p:.2g}")(*stats.pearsonr(self.movieData['critic_rating'],
                                                                                              self.movieData['audience_rating'])), transform=j.ax_joint.transAxes)
    ...
    def getTop10(self,data):
        p=sns.catplot(aspect=1.5,y='title',x='audience_rating',data=data.sort_values(['audience_rating','critic_rating'],ascending=False).drop(['genre'],axis=1).drop_duplicates().head(10),palette="ch:.25",kind='bar')
        p.set(xlim=(10,100))
    ...
    def getTop10HighBudgetMovie(self,data):
        sns.catplot(aspect=1.5,y='title',x='budget_in_millions',data=data.sort_values(['budget_in_millions'],ascending=False).drop(['genre'],axis=1).drop_duplicates().head(10),palette="ch:.25",kind='bar')
        sns.set_style("ticks",{"xtick.major.size":8,"ytick.major.size":8})
        ...
        while True:            
            ...
            if flag==1 and Choice=='':
                ...
                if Choice == 's':
                    ...
                    self.optionAnalyze={1:'Display Top 10 Rated Movies',
                                        2:'Display Top 10 High Budget Movies',
